...
def register(worker: ZeebeWorker, client: ZeebeClient):
    @worker.task(task_type="sendShortlistedCandidatesWBIG")
    async def sendShortlistedCandidatesWBIG(job: Job):
        """Send information for new job opening to WBIG"""
        logging.info("Message 'sendShortlistedCandidatesWBIG' triggered")
        ...
        await client.publish_message(
            name="testJobInformationChangeRequest",#receivePotentialCandidates
